...
const data = {
   "posts":{
      "edges":[
         {
            "node":{
               "excerpt":"Nested structures are useful for managing and editing. But when it comes to representation in UI, it might\nnot be trivial to iterate through…",
               "fields":{
         ...
         {
            "node":{
               "excerpt":"Linking web fonts in SASS when using Webpack might not work as expected. The most common problem is incorrect URL resolving if a font is…",
               "fields":{
         ...
         {
            "node":{
               "excerpt":"In these unique times, well-organized remote work becomes one of the key factors for success. This applies to any kind of work, and managing…",
               "fields":{
